...
(*                                                                         *)
(* The lift stops at the blacklist. failOpen remains a single BOOLEAN,      *)
(* so this module checks a SYNCHRONISED fail-open: either every guard is    *)
(* enforcing or none is. Deployment decides it per guard and per call, so   *)
(* the mechanised conditional is the synchronised case of it. Table III in  *)
(* the paper scopes the claim the same way.                                 *)
(*                                                                         *)
