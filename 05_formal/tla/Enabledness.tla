----------------------------- MODULE Enabledness -----------------------------
(***************************************************************************)
(* Model-checked support for the two enabledness facts that Liveness.tla     *)
(* is forced to take as AXIOMs.                                             *)
(*                                                                          *)
(* Liveness.tla proves  SpecL => <>HasLeader  with all 234 obligations       *)
(* discharged EXCEPT two, which are stated there as                          *)
(*                                                                          *)
(*     AXIOM ElectEnabled ==                                                *)
...
(*                                                                          *)
(* They are axioms not because the content is hard but because TLAPS's       *)
(* ExpandENABLED is not operational in any available build (even the trivial *)
(* ENABLED <<x' = x+1>>_vars fails under zenon, z3 and Isabelle).            *)
(*                                                                          *)
(* TLC, unlike TLAPS, CAN evaluate ENABLED.  This module restates the two    *)
(* axioms verbatim as state predicates so TLC checks them over the same      *)
(* bounded model used elsewhere in the paper (N=5, F=2, MaxTerm=3).  That    *)
(* converts "axiom" into "axiom, machine-checked over the bounded model" --  *)
(* strictly more than a prose appendix proof would give.                     *)
(*                                                                          *)
