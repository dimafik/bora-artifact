...
#
# Usage: xhost_caliper_net.sh <tps> <seconds> <label> [workers] [network.yaml]
set -u
TPS="${1:?tps}"; DUR="${2:?seconds}"; LABEL="${3:?label}"; W="${4:-4}"; NET="${5:-fabric-xhost.yaml}"
...
sudo docker run --rm --name caliper-xb $ADDH \
  ...
  -e CALIPER_NETWORKCONFIG=networks/$NET \
