...
echo "===== forced-election: target=orderer3, baseline [] vs BORA [3], ${SEEDS}seed x ${NE}elec ====="
...
# Non-destructive heal: in a SINGLE exec per orderer, start the sidecar ONLY if its
# process is genuinely absent (never pkill a live one; rm the stale socket only when
# we are about to restart). Tolerates transient docker-exec failures (|| true) so a
# broken-pipe hiccup can never cascade into killing healthy sidecars.
...
# Read cluster-wide current leader from the globally-latest "Raft leader changed: X -> Y"
# (INFO-level, etcdraft chain.go). Node id Y == orderer index (consenters listed orderer1..N).
# Fabric timestamps "YYYY-MM-DD HH:MM:SS.mmm UTC" sort lexicographically by time, so the last
# line after sorting all orderers' logs is the most recent leadership event. "-> 0" == no leader.
...
RUN=/tmp/ns_heal; touch $RUN; ( while [ -f $RUN ]; do heal; sleep 3; done ) & HP=$!
echo "label,N,seed,o3_wins,elec,live" > "$OUT/results.csv"
# Force re-election by PAUSING the current leader (cgroup freezer) for > election
# timeout, then UNPAUSING it. Pause freezes the orderer without killing the exec'd
# sidecar, and never touches the other orderers -- so suppression stays enforced by
# the live voters throughout. We read the new leader while the old one is still frozen.
phase(){ WINS=0; LIVE=0; local k L LC NL; for k in $(seq 1 "$NE"); do
...
for s in $(seq 1 "$SEEDS"); do
  set_all "[]"; sleep 2; phase "base_s$s"; echo "  N=$N base s$s: o3 $WINS/$NE live $LIVE/$NE" | tee -a "$OUT/summary.txt"
  echo "base,$N,$s,$WINS,$NE,$LIVE" >> "$OUT/results.csv"
  set_all "[3]"; sleep 2; phase "bora_s$s"; echo "  N=$N BORA s$s: o3 $WINS/$NE live $LIVE/$NE" | tee -a "$OUT/summary.txt"
  echo "bora,$N,$s,$WINS,$NE,$LIVE" >> "$OUT/results.csv"
done
set_all "[]"; rm -f $RUN; kill $HP 2>/dev/null || true
echo "NSWEEP_N${N}_DONE"; cat "$OUT/results.csv"
