...
#
#   r13_p4.sh <N> <seeds> <dwell-seconds> [fp-rates]   (v5 binary required)
set -u
N="${1:?need N}"; SEEDS="${2:-6}"; DWELL="${3:-300}"; NELEC=0; DUR=0
read -r -a RATES <<< "${4:-10 20}"
HOLD_S="${HOLD_S:-10}"; COOL_S="${COOL_S:-60}"
F=$(( (N - 1) / 2 ))
CAP=$(( F - 1 ))
TARGET=3
ELEC_EVERY=30
...
OUT="$D/results/r13p4_N${N}_$(date +%m%d-%H%M%S)"; mkdir -p "$OUT"
echo "arm,rate,seed,dwell_s,demotions,demote_TP,demote_FP,target_tenure_s,leaderless_s,target_leader_at_end,seat_attempts,settle_s,repaired,safety_viol" \
  > "$OUT/cells.csv"
...
# Arm P: the same eviction as D, behind two conditions an operator would set --
# the advice has to name the leader for HOLD_S seconds without a gap, and no more
# than one demotion may happen per COOL_S seconds. Both only ever SUPPRESS what D
# would do, so the arms are directly comparable.
policy_loop(){ local logf="$1" since_in=0 last_dem=0 now held
  ( while [ -f /tmp/r13.on ]; do
      [ -f /tmp/r13.busy ] && { sleep 0.5; continue; }
      bl=$(bl_of_file "$BTFP"); L=$(leader_now)
      now=$(date +%s)
      if [ -n "$bl" ] && [ "$L" != 0 ] && printf ' %s ' "$bl" | grep -q " $L "; then
        [ "$since_in" = 0 ] && since_in="$now"
        held=$(( now - since_in ))
        if [ "$held" -ge "$HOLD_S" ] && [ $(( now - last_dem )) -ge "$COOL_S" ]; then
          tset=$(grep -oE '"true" *: *\[[0-9, ]*\]' "$BTFP" 2>/dev/null | grep -oE '[0-9]+' | tr '\n' ' ')
          cause=FP; printf ' %s ' "$tset" | grep -q " $L " && cause=TP
          echo "$(date +%s.%N) demote leader=$L cause=$cause held=${held}s bl=[$bl] true=[$tset]" >> "$logf"
          docker pause "$(cont "$L")" >/dev/null 2>&1; sleep 9
          docker unpause "$(cont "$L")" >/dev/null 2>&1; sleep 4
          last_dem=$(date +%s); since_in=0
        fi
      else
        since_in=0
      fi
...
# The Active-Leader Rule is a property of the binary now, not of the harness: the
# switch file makes the tick guard apply to the incumbent as well.
alr_switch(){   # on|off
  for o in "${ORD[@]}"; do
    if [ "$1" = on ]; then docker exec "$o" sh -c 'touch /tmp/bora-alr-off' 2>/dev/null
    else docker exec "$o" sh -c 'rm -f /tmp/bora-alr-off' 2>/dev/null; fi
  done
}

# Arm P: the same advice, filtered. The leader is allowed into B_t only after the
# advisor has named it for HOLD_S seconds without a gap, and not within COOL_S of
# the last time the filter let one through. Everything else passes untouched, so
# the only difference from arm D is when the incumbent appears in the set.
push_policy(){ local src="$1" logf="$2" since_in=0 last_dem=0
  ( while [ -f /tmp/r13.on ]; do
      bl=$(grep -oE '"blacklist" *: *\[[0-9, ]*\]' "$src" 2>/dev/null | grep -oE '\[[0-9, ]*\]' | tr -d ' ')
      if [ -n "$bl" ]; then
        L=$(cat /tmp/r13.leader 2>/dev/null || echo 0); now=$(date +%s)
        inb=0; [ "$L" != 0 ] && printf '%s' "$bl" | grep -qE "[\[,]${L}[],]" && inb=1
        if [ "$inb" = 1 ]; then
          [ "$since_in" = 0 ] && since_in="$now"
          held=$(( now - since_in ))
          if [ "$held" -ge "$HOLD_S" ] && [ $(( now - last_dem )) -ge "$COOL_S" ]; then
            tset=$(grep -oE '"true" *: *\[[0-9, ]*\]' "$src" 2>/dev/null | grep -oE '[0-9]+' | tr '\n' ' ')
            cause=FP; printf ' %s ' "$tset" | grep -q " $L " && cause=TP
            echo "$(date +%s.%N) release leader=$L cause=$cause held=${held}s bl=$bl true=[$tset]" >> "$logf"
            last_dem="$now"; since_in=0
          else
            bl=$(printf '%s' "$bl" | sed -E "s/\[${L},/[/; s/,${L}\]/]/; s/\[${L}\]/[]/")
          fi
        else
          since_in=0
        fi
...
# Arm D records what the binary did, so the demote log is written by watching the
# leader change while the leader was in B_t rather than by the harness acting.
watch_demotes(){ local logf="$1" prev=0
  ( while [ -f /tmp/r13.on ]; do
      L=$(leader_now)
      if [ "$prev" != 0 ] && [ "$L" != 0 ] && [ "$L" != "$prev" ]; then
        bl=$(bl_of_file "$BTFP"); tset=$(grep -oE '"true" *: *\[[0-9, ]*\]' "$BTFP" 2>/dev/null | grep -oE '[0-9]+' | tr '\n' ' ')
        if printf ' %s ' "$bl" | grep -q " $prev "; then
          cause=FP; printf ' %s ' "$tset" | grep -q " $prev " && cause=TP
          echo "$(date +%s.%N) demote leader=$prev cause=$cause new=$L bl=[$bl] true=[$tset]" >> "$logf"
        fi
      fi
      [ "$L" != 0 ] && prev="$L"
      sleep 0.5
...
# What the cluster was actually told, once a second. Arm P publishes a filtered
# blacklist, so the injector log is its intent and this is the fact.
advice_watch(){   # <cell dir>
  local cdir="$1" o
  o=$(cont 1)
  ( while [ -f /tmp/r13.on ]; do
      printf '%s,%s\n' "$(date +%s)" \
        "$(docker exec "$o" cat /tmp/bora-advice.json 2>/dev/null | tr -d '\n' | tr ',' ';')" \
        >> "$cdir/advice_samples.csv"
      sleep 1
    done ) >/dev/null 2>&1 & ADVPID=$!
}

# Cells start from the same state: the target must hold the term for 30 straight
# 2 s samples before the window opens, whatever the seating cost to get there.
settle(){   # -> seconds spent settling, or 999 on failure
  local ok=0 t=0 L
  while [ "$t" -lt 180 ]; do
    L=$(leader_now); t=$(( t + 2 )); sleep 2
    if [ "$L" = "$TARGET" ]; then ok=$(( ok + 1 )); else ok=0; fi
    [ "$ok" -ge 30 ] && { echo "$t"; return 0; }
  done
  echo 999; return 1
}
...
log "R1-2 Phase 4 (v5 binary, system-side demotion)  N=$N seeds=${SEED_FROM:-1}..$SEEDS dwell=${DWELL}s rates=${RATES[*]} hold=${HOLD_S}s cooldown=${COOL_S}s"
log "  question: with the degraded orderer AS leader, what does each eviction policy do?"
log "OUT=$OUT"
start_probe; start_attack; sleep 20
bash "$D/alg1/start_daemon.sh" "$N" "$F" | tee -a "$OUT/run.log"
sleep 25; gate_daemon; check_attack

for seed in $(seq "${SEED_FROM:-1}" "$SEEDS"); do
  for rate in "${RATES[@]}"; do
    for arm in A D P; do
      check_attack; gate_daemon
      cdir="$OUT/${arm}_p${rate}_s${seed}"; mkdir -p "$cdir"
      : > "$cdir/leader_samples.csv"
      health_check || { log "  SKIP ${arm}_p${rate}_s${seed} (health)"; continue; }
      REP=$(cat /tmp/r13.repaired 2>/dev/null || echo 0)
      log "cell ${arm}_p${rate}_s${seed}"

      if seat_target; then ST=$(cat /tmp/r13.seattries 2>/dev/null || echo 0); log "  target seated after $ST attempts"
      else log "  SKIP ${arm}_p${rate}_s${seed} (seat failed)"
        echo "$arm,$rate,$seed,$DWELL,-,-,-,-,-,-,$(cat /tmp/r13.seattries 2>/dev/null),$REP,SEAT_FAIL" >> "$OUT/cells.csv"
        continue; fi

      rm -f "$BTFP" /tmp/r13.busy; touch /tmp/r13.on
      since=$(date -u +%Y-%m-%dT%H:%M:%SZ)
      WATCHPID=""; leader_watch "$cdir"; sleep 2
      python3 "$D/alg1/r13_fp_inject.py" --rate "$rate" --seed "$seed" --dur $(( DWELL + 120 )) \
          --n "$N" --target "$TARGET" --cap "$CAP" --hold 20 \
          --src "$BT" --out "$BTFP" --log "$cdir/fp.log" >/dev/null 2>"$cdir/fp.err" &
      FPPID=$!
      gate_w=0
      while [ ! -s "$BTFP" ] && [ "$gate_w" -lt 20 ]; do sleep 1; gate_w=$(( gate_w + 1 )); done
      [ -s "$BTFP" ] || { log "  GATE FAIL: injector silent after ${gate_w}s"; tail -3 "$cdir/fp.err" 2>/dev/null | tee -a "$OUT/run.log"; rm -f /tmp/r13.on; kill "$FPPID" 2>/dev/null; exit 1; }

      # Settle FIRST, in the shipped configuration: switch off, empty advice, the
      # target holding the term for 30 straight samples. Turning an eviction arm
      # on before this point makes the gate unsatisfiable, because the target is
      # in B_t from the first advice and steps down as soon as it takes the term.
      PUSHPID=""; AUTHPID=""
      alr_switch off; set_all "[]"
      SETTLE=$(settle) || { log "  SKIP ${arm}_p${rate}_s${seed} (target did not settle)"
        rm -f /tmp/r13.on; kill "$FPPID" "$WATCHPID" 2>/dev/null || true
        alr_switch off; set_all "[]"
        echo "$arm,$rate,$seed,$DWELL,-,-,-,-,-,-,$ST,$SETTLE,$REP,SETTLE_FAIL" >> "$OUT/cells.csv"; continue; }
      log "  settled after ${SETTLE}s"
      SINCE_WIN=$(date +%s)

      ADVPID=""; advice_watch "$cdir"
      case "$arm" in
        A) alr_switch off; push_from "$BTFP" ;;
        D) alr_switch on;  push_from "$BTFP"; watch_demotes "$cdir/demote.log" ;;
        P) alr_switch on;  push_policy "$BTFP" "$cdir/release.log"; watch_demotes "$cdir/demote.log" ;;
      esac

      sleep "$DWELL"

      rm -f /tmp/r13.on /tmp/r13.busy; sleep 2
      kill "$FPPID" "$PUSHPID" "$WATCHPID" $AUTHPID $ADVPID 2>/dev/null || true
      alr_switch off; set_all "[]"

      LESS=$(awk -F, -v w="$SINCE_WIN" '$1>=w && $2==0{n++} END{print (n+0)*2}' "$cdir/leader_samples.csv")
      TEN=$(awk -F, -v w="$SINCE_WIN" -v t="$TARGET" '$1>=w && $2==t{n++} END{print (n+0)*2}' "$cdir/leader_samples.csv")
      ENDL=$(tail -1 "$cdir/leader_samples.csv" | cut -d, -f2)
      DEM=0; DTP=0; DFP=0
      if [ -f "$cdir/demote.log" ]; then
        DEM=$(grep -c ' demote ' "$cdir/demote.log" || true)
        DTP=$(grep -c 'cause=TP' "$cdir/demote.log" || true)
      ...
      SV=0; for o in "${ORD[@]}"; do SV=$((SV + $(docker logs --since "$since" "$o" 2>&1 | grep -acE 'PANI|FATAL' || true))); done
      echo "$arm,$rate,$seed,$DWELL,$DEM,$DTP,$DFP,$TEN,$LESS,$ENDL,$ST,$SETTLE,$REP,$SV" >> "$OUT/cells.csv"
      log "  -> demote=$DEM (TP=$DTP FP=$DFP) target_tenure=${TEN}s leaderless=${LESS}s end_leader=$ENDL safety=$SV"
    done
  done
done
