...
threshold daemon (`predictor_daemon_meanrtt_wsl.py`), all at r = 0 except 15
bring-up cycles at N = 15 that read every orderer as down (r = 15), where the
`max(f - r - 1, 0)` clamp held the cap at 0. The original
