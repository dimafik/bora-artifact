...
#
#   r13_p3.sh <N> <seeds> <dwell-seconds> [fp-rates]
set -u
N="${1:?need N}"; SEEDS="${2:-2}"; DWELL="${3:-300}"; NELEC=0; DUR=0
...
OUT="$D/results/r13p3_N${N}_$(date +%m%d-%H%M%S)"; mkdir -p "$OUT"
echo "arm,rate,seed,dwell_s,demotions,demote_TP,demote_FP,target_tenure_s,leaderless_s,target_leader_at_end,seat_attempts,repaired,safety_viol" \
  > "$OUT/cells.csv"
...
log "R1-2 Phase 3  N=$N seeds=$SEEDS dwell=${DWELL}s rates=${RATES[*]} hold=${HOLD_S}s cooldown=${COOL_S}s"
  ...
  for rate in "${RATES[@]}"; do
    for arm in A D P; do
      ...
      case "$arm" in
        A) push_from "$BTFP" ;;
        D) push_from "$BTFP"; noalr_loop "$cdir/demote.log" ;;
        P) push_from "$BTFP"; policy_loop "$cdir/demote.log" ;;
      esac
      ...
      SV=0; for o in "${ORD[@]}"; do SV=$((SV + $(docker logs --since "$since" "$o" 2>&1 | grep -acE 'PANI|FATAL' || true))); done
      echo "$arm,$rate,$seed,$DWELL,$DEM,$DTP,$DFP,$TEN,$LESS,$ENDL,$ST,$REP,$SV" >> "$OUT/cells.csv"
