...
ORD=(orderer.example.com orderer2.example.com orderer3.example.com \
     orderer4.example.com orderer5.example.com)
name_for_id(){ case $1 in 1) echo orderer.example.com;; *) echo "orderer$1.example.com";; esac; }

OUT="$R/r25b_$(date +%Y%m%d-%H%M%S)"; mkdir -p "$OUT"
say(){ echo "$@" | tee -a "$OUT/summary.txt"; }
say "R2-5 leader cost (2nd attempt): delay=${DLY}ms max_pin=$MAXPIN"

leader_id(){ docker logs --tail 400 orderer.example.com 2>&1 \
  | grep -ao "Raft leader changed:[^c]*-> [0-9][0-9]*" | tail -1 | grep -ao "[0-9][0-9]*$"; }
