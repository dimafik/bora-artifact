...
class Attributor(nn.Module):
    ...
    def forward(s, X):
        """Relation BEFORE temporal pooling.

        The first version pooled each node over time and only then compared
        nodes, which is the same fault found in R2 v1 and fixed in R2b: a
        time-averaged embedding no longer carries the waveform the comparison
        needs.  The hand-built mean-deviation feature scored 0.711 precisely
        because it subtracts at the raw-signal level, before any pooling."""
        B, N = X.shape[0], X.shape[1]
        Z = s.n(X.reshape(B * N, K, 8))
        if s.rel == "mean-dev":
            Z4 = Z.reshape(B, N, K, 8)
            Z = (Z4 - Z4.mean(1, keepdim=True)).reshape(B * N, K, 8)
        h = s.enc(Z.transpose(1, 2)).transpose(1, 2)              # B*N, K, D
        if s.rel in ("conv2d", "gru-nodes", "attention"):
            hn = h.reshape(B, N, K, D).permute(0, 2, 1, 3)        # B, K, N, D
            if s.rel == "conv2d":
                m = s.mix(hn.reshape(B * K, N, D).permute(0, 2, 1).unsqueeze(-1))
                m = m.squeeze(-1).permute(0, 2, 1)
            elif s.rel == "gru-nodes":
                m = s.mix(hn.reshape(B * K, N, D))[0]
            else:
                x = hn.reshape(B * K, N, D)
                m = s.mix(x, x, x)[0]
            hn = (hn.reshape(B * K, N, D) + m).reshape(B, K, N, D)
            h = hn.permute(0, 2, 1, 3).reshape(B * N, K, D)
        z = h.mean(1).reshape(B, N, D)                            # pool AFTER
        return s.o(z).squeeze(-1)
...
def main():
    ...
    for rel in RELS:
        ...
        rows.append(best)
        json.dump(rows, open("d1b_results.json", "w"), indent=1)
