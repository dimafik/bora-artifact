...
def pgd(target, rho_min, lr, seed, init, steps=STEPS):
    """Projected gradient descent inside the threat model.

    The soft-penalty version this replaces let the optimiser buy its way out of
    the feasible set: 26 of 32 worst cases in the first run violated the
    autocorrelation floor, one returning -0.85 where 0.8 was required.  Here the
    iterate is projected back after every step, so nothing infeasible is ever
    evaluated and the reported number is a legal attack by construction.
    """
    rng = np.random.default_rng(seed)
    r0 = project.project(torch.tensor(init_raw(init, N_ATK, rng)), rho_min)
    r = r0.clone().requires_grad_(True)
    opt = torch.optim.Adam([r], lr=lr)
    for _ in range(steps):
        opt.zero_grad()
        target.score_raw(r).mean().backward()
        opt.step()
        with torch.no_grad():
            r.copy_(project.project(r.detach(), rho_min))
    rr = project.project(r.detach(), rho_min)
    return (rr.numpy(), float(project.autocorr1(rr).mean()),
            float(rr.mean()), float(rr.std(1).mean()),
            project.feasible(rr, rho_min))
    ...
    for m in models:
        ...
        for rho in RHOS:
            ...
            for lr in LRS:
                for seed in SEEDS:
                    for init in INITS:
                        raw, ac, mu, sd, feas = pgd(atk, rho, lr, seed, init)
                        a = gen.auc(np.r_[np.zeros(N_ATK), np.ones(N_ATK)],
                                    np.r_[h, m.score_np(raw)])
                        combos.append(dict(lr=lr, seed=seed, init=init,
                                           auc=round(a, 4), autocorr=round(ac, 3),
                                           mean=round(mu, 3), std=round(sd, 3),
                                           feasible_frac=round(feas, 3)))
            best = min(combos, key=lambda c: c["auc"])
            sweep["rho_%.1f" % rho] = dict(worst_auc=best["auc"], best_combo=best,
                                           all_combos=combos)
            print("  %-32s rho=%.1f  worst AUC=%.4f  (init=%s, ac=%.2f, feasible=%.0f%%)"
                  % (m.name, rho, best["auc"], best["init"], best["autocorr"],
                     100 * best["feasible_frac"]), flush=True)
        rows.append(dict(name=m.name, params=m.params, differentiable=m.differentiable,
                         non_adaptive=round(na, 4),
                         worst_case=round(min(v["worst_auc"] for v in sweep.values()), 4),
                         sweep=sweep, minutes=round((time.time() - te) / 60, 1)))
        json.dump(rows, open("panel2_smoke.json" if "--smoke" in sys.argv else "panel2_results.json", "w"), indent=1)
