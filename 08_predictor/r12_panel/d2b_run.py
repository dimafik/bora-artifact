...
class Net(nn.Module):
    ...
    def forward(s, X):
        """Relation before temporal pooling, matching the corrected R2b."""
        B, N = X.shape[0], X.shape[1]
        h = s.enc(s.n(X.reshape(B * N, K, 8)).transpose(1, 2)).transpose(1, 2)
        hn = h.reshape(B, N, K, D).permute(0, 2, 1, 3).reshape(B * K, N, D)
        if s.rel in ("mlp-concat", "conv2d") and N != s.ntrain:
            hn = torch.nn.functional.adaptive_avg_pool1d(
                hn.transpose(1, 2), s.ntrain).transpose(1, 2)
        if s.rel == "mlp-concat":
            z = s.mix(hn).reshape(B, K, -1).mean(1)
        elif s.rel == "conv2d":
            z = s.mix(hn.permute(0, 2, 1).unsqueeze(-1)).squeeze(-1).squeeze(-1)
            z = z.reshape(B, K, -1).mean(1)
        elif s.rel == "gru-nodes":
            z = (hn + s.mix(hn)[0]).mean(1).reshape(B, K, -1).mean(1)
        else:
            z = (hn + s.mix(hn, hn, hn)[0]).mean(1).reshape(B, K, -1).mean(1)
        return s.o(z)
...
def main():
    ...
    for rel in RELS:
        ...
        rows.append(best)
        json.dump(rows, open("d2b_results.json", "w"), indent=1)
